...
vi.mock('../../src/services/AIService', async () => {
  ...
  return {
    ...
    AIService: class MockAIService {
      ...
      async sendConversation(messages: any[], options: any = {}) {
        return {
      ...
      async extractData(text: string, schema: string, options: any = {}) {
        return { extracted: 'data' };
      }

      async extractStructuredData(text: string, schema: any, options: any = {}) {
        return { structured: 'data' };
      }

      async extractRecipeData(text: string, options: any = {}) {
        return {
          title: 'Mock Recipe',
      ...
      async summarize(text: string, maxLength: number = 200, options: any = {}) {
        return 'Mock summary';
      }
...
describe('AIService (Mocked)', () => {
  ...
  describe('extractData', () => {
    it('should extract data and return parsed JSON', async () => {
      const service = new AIService();
      const result = await service.extractData<{ name: string; age: number }>(
